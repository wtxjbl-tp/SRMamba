...
class LinearUpsampler(nn.Module):
    def __init__(self, scale_factor, mode='bilinear', align_corners=False):
        ...
        :param mode: 插值模式，可选 'nearest', 'linear', 'bilinear', 'trilinear', 'bicubic'
        :param align_corners: 对应 F.interpolate 参数
        """
        super().__init__()
    ...
    def forward(self, x):
        """
        :param x: 输入张量, shape (B, C, H, W) 或 (B, C, L) 等
        :return: 上采样后的张量
        """
...
class VSSM(nn.Module):
    def __init__(
        ...
        for i_layer in range(self.num_layers - 1):
            downsample = _make_downsample(
                ...
                channel_first=self.channel_first,
            ) if (i_layer < self.num_layers - 2) else nn.Identity()
    ...
    def skip_connection(self):
        ...
        for i in range(self.num_layers - 1):
            dim = [dim * 2 for dim in self.dims]
            layer = nn.Conv2d(dim[i] , dim[i+1], kernel_size=1)
    ...
    def forward(self, x: torch.Tensor, x_save):
...
# =====================================================
def vmamba_up_tiny_s1l8(depths, embed_dim,channel_first=True):
    return VSSM(
        depths=depths, dims=embed_dim, drop_path_rate=0.2,
        patch_size=4, in_chans=3, num_classes=1000,
        ...
        mlp_ratio=4.0, mlp_act_layer="gelu", mlp_drop_rate=0.0, gmlp=False,
        patch_norm=True, norm_layer=("ln2d" if channel_first else "ln"),
        downsample_version="up", patchembed_version="v2",
        use_checkpoint=False, posembed=False, imgsize=64,
    )
