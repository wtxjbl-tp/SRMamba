...
def train_one_epoch(model: torch.nn.Module,
    ...
    DATA=metric_logger.log_every(data_loader, print_freq, header)
...
@torch.no_grad()
def Upsamping_only(data_loader, model, device, log_writer, args=None):
    '''No Evelation
        only upsampling
    '''
    ...
    for batch in tqdm.tqdm(data_loader):
        ...
        with torch.cuda.amp.autocast():
            ...
            pred_img_iteration = torch.empty(iteration, images_high_res.shape[1], images_high_res.shape[2],
                                             images_high_res.shape[3]).to(device)
            for i in range(int(np.ceil(iteration / iteration_batch))):
                input_batch = iteration_batch if (iteration - i * iteration_batch) > iteration_batch else (
                            iteration - i * iteration_batch)
                test_imgs_input = torch.tile(images_low_res, (input_batch, 1, 1, 1))
