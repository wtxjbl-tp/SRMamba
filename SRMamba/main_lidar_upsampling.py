...
import model.SRMamba as SRMamba
from engine_upsampling import train_one_epoch, evaluate, get_latest_checkpoint, MCdrop, Upsamping_only
import wandb
...
def get_args_parser():
    ...
    parser.add_argument('--model_select', default='mae', type=str,
                        choices=['SRMamba_small','SRMamba_tiny' ,'SRMamba_medium','SRMamba_large'])
    ...
    # Dataset parameters
    parser.add_argument('--dataset_select', default='durlar', type=str, choices=['kitti','nuScenes'])
...
def main(args):
    ...
    # define the model
    model = SRMamba.__dict__[args.model_select](img_size = tuple(args.img_size_low_res),
    ...
    if args.eval and os.path.exists(args.output_dir):
        ...
        # args.mc_drop = False
        if args.mc_drop: # True
            print("Evaluation with Monte Carlo Dropout")
            Upsamping_only(data_loader_val, model, device, log_writer = log_writer, args = args)
